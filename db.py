...
def add_student(name: str, price: float) -> int:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO tb_students (name, price) VALUES (%s, %s) RETURNING id",
                (name, price),
            )
...
def export_all() -> dict:
    """Выгружает всех учеников, расписание и уроки в обычный словарь,
...
def restore_all(data: dict):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("TRUNCATE tb_lessons, tb_schedule_slots, tb_students RESTART IDENTITY CASCADE")

            for s in data.get("students", []):
                cur.execute(
                    """
                    INSERT INTO tb_students (id, name, price, is_active, created_at)
                    VALUES (%(id)s, %(name)s, %(price)s, %(is_active)s, %(created_at)s)
                    """,
                    s,
                )
            ...
            for l in data.get("lessons", []):
                cur.execute(
                    """
                    INSERT INTO tb_lessons (id, student_id, lesson_date, lesson_time, price, conducted, created_at)
                    VALUES (%(id)s, %(student_id)s, %(lesson_date)s, %(lesson_time)s, %(price)s, %(conducted)s, %(created_at)s)
                    """,
                    l,
                )
            ...
            for table in ("tb_students", "tb_schedule_slots", "tb_lessons"):
                cur.execute(
